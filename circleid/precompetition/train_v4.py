#!/usr/bin/env python3
"""
train_v4.py -- Writer Identification

1. Uses ALL data: train.csv + additional_train.csv (34,650 known + 5,600 real OOD)
2. Targets new test.csv (5,905 v2_ samples)
3. Real OOD samples for threshold calibration (not simulated)
4. NetVLAD aggregation on DINOv2 patch tokens (SOTA for writer ID)
5. Scanner-bridging augmentations (domain shift: train ~80px, test ~150px)
6. Foreground token filtering (only ink patches feed VLAD)

Architecture: DINOv2 ViT-B/14 (frozen) + LoRA + NetVLAD + ArcFace + OOD calibration
"""
...
class Config:
    ...
    output_dir: Path = Path("./outputs_v4_vlad")
    checkpoint_dir: Path = Path("./checkpoints_v4_vlad")
    ...
    eval_every_n_epochs: int = 3  # split phase: eval every N epochs 

    # -- OOD ----------------------------------------------------------
    ood_alpha: float = 0.7
    ood_train_lambda: float = 0.5  # outlier exposure weight
    ood_train_warmup: int = 5  # start OOD loss after this epoch
...
def worker_init_fn(worker_id):
    """Ensure each DataLoader worker has a unique RNG seed."""
...
def _apply_scanner_bridging_impl(image, rng=None):
    ...
    # 1. Resolution simulation (CRITICAL — train ~90px, test ~130px, different scanners)
    if rng.random() < 0.5:
    ...
    # 3. Random Gaussian blur/sharpen (different scanner optics)
    if rng.random() < 0.3:
        if rng.random() < 0.5:
...
def preprocess_image(image, mode="cv2", image_size=224,
    ...
    # Simple resize (no ink crop) -- preserves spatial context for DINOv2
...
def extract_handcrafted_features(image, mode="cv2", n_fourier=16, n_radial_bins=16):
    """Extract 64-dim handcrafted geometric/texture features from a circle image.

    Groups: Shape(8) + StrokeWidth(4) + Curvature(6) + InkTexture(8) + Fourier(16) + Radial(22) = 64
    Returns np.float32[64], zeros on failure, NaN-safe.
    """
    ...
    try:
        ...
        pass  # return zeros on any failure
...
class WriterDataset(Dataset):
    def __init__(self, df, indices, data_dir, writer2idx,
        ...
        # Cache polar strips and handcrafted features (computed from raw image, constant across epochs)
        self._polar_cache = {}
        self._hc_cache = {}
    ...
    def __getitem__(self, idx):
        ...
        # Handcrafted features from original image (cached — depends only on raw image)
        hc_feats = None
        ...
        # Polar strip from original image (cached base, augmentation applied on top)
        polar_strip = None
        if self.use_polar:
            ...
            # Apply circular shift augmentation on cached strip (training only)
...
class WriterTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Polar strip (cached — no augmentation on test)
        polar_strip = None
        if self.use_polar:
...
class PenAxisBatchSampler:
    ...
    def __init__(self, df, indices, writer2idx, num_writers=4,
        ...
        # Only writers with >= pens_per_writer pens are eligible
...
class NetVLAD(nn.Module):
    ...
    def init_clusters(self, patch_descriptors):
        """Initialize centroids + soft-assignment conv via K-means on real patch tokens.
        Must be called BEFORE training. patch_descriptors: [M, D] tensor on CPU.
        """
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            effective_mask = mask_bool | ~has_fg  # all-bg samples → all True (use everything)
            soft_assign = soft_assign.masked_fill(~effective_mask, float('-inf'))
        soft_assign = F.softmax(soft_assign, dim=2)  # [B, K, N]

        # VLAD core: loop over clusters to avoid [B, K, N, D] tensor (saves ~4x memory)
...
def get_factorial_lambda(epoch, warmup_start=5, warmup_end=15, lambda_max=0.5):
    """Linear ramp: 0 before warmup_start, linear to lambda_max at warmup_end, constant after."""
    if epoch < warmup_start:
        return 0.0
    if epoch >= warmup_end:
        return lambda_max
...
def log_batch_pair_stats(epoch_stats, epoch, lambda_fac):
    """Print accumulated factorial contrastive stats for an epoch."""
...
def eval_val_loss(model, val_loader, device, use_amp=False):
    """Quick validation: ArcFace loss + cosine accuracy. Returns (loss, acc)."""
    model.eval()
...
# ======================================================================
# Main: Data Exploration -> Training -> Calibration -> Submission
# ======================================================================
...
def main():
    ...
    for split in splits:
        ...
        # Load whichever checkpoint (EMA or non-EMA) had the best AUROC
    ...
    # -- Step 10: 3-Seed Ensemble Submission ----------------------------
    final_cfg = Config()
    ...
    for seed_i, seed_val in enumerate(cfg.ensemble_seeds):
        ...
        for epoch in range(cfg.epochs):
            ...
            # Early stopping check using EMA model on validation set
        ...
        # Apply best EMA weights (or latest if no early stopping triggered)
        if best_ema_state is not None:
            ema_f.shadow = best_ema_state
            ema_f.apply_shadow(model)
        elif ema_f_initialized:
            ema_f.apply_shadow(model)
        # else: EMA never initialized (early stopped before ema_start_epoch), use raw weights
        model.eval()
        ...
        # TTA inference for this seed (logits + cosine + embeddings in one pass)
        torch.cuda.empty_cache()
